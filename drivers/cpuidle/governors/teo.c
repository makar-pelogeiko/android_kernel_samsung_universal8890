...
static void teo_update(struct cpuidle_driver *drv, struct cpuidle_device *dev)
{
    ...
    ktime_t temp_holder = {cpu_data->sleep_length_ns};
	unsigned int sleep_length_us = ktime_to_us(temp_holder);
	...
	if (cpu_data->time_span_ns >= cpu_data->sleep_length_ns) {
        ...
        ktime_t temp_holder_time_span_ns = {cpu_data->time_span_ns};
		measured_us = ktime_to_us(temp_holder_time_span_ns);
		/*
 ...
 * DELETED becouse of no interface in struct cpuidle_governor
 */
static int teo_select(struct cpuidle_driver *drv, struct cpuidle_device *dev)
{
    ktime_t temp_sleep_length_ns = tick_nohz_get_sleep_length();
	struct teo_cpu *cpu_data = per_cpu_ptr(&teo_cpus, dev->cpu);
 int latency_req = pm_qos_request(PM_QOS_CPU_DMA_LATENCY);
//my_need had to be this	int latency_req = cpuidle_governor_latency_req(dev->cpu);
...
//changed	cpu_data->sleep_length_ns = tick_nohz_get_sleep_length(&delta_tick);
    
    cpu_data->sleep_length_ns = (u64)(temp_sleep_length_ns.tv64);
	duration_us = ktime_to_us(temp_sleep_length_ns);
	 ...
	 */
    //here was CPUIDLE_FLAG_POLLING instead of CPUIDLE_FLAG_TIME_VALID
	if (((drv->states[idx].flags & CPUIDLE_FLAG_TIME_VALID) ||
