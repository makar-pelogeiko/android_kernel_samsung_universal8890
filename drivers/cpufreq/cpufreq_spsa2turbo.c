/*
 *  drivers/cpufreq/cpufreq_spsa2turbo.c
 *
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_SPSA2TURBO
static struct cpufreq_governor cpufreq_gov_spsa2turbo;
#endif

static unsigned int default_powersave_bias;

static void spsa2turbo_powersave_bias_init_cpu(int cpu)
{
	struct od_cpu_dbs_info_s *dbs_info = &per_cpu(od_cpu_dbs_info, cpu);

    pr_warn("gov spsa2 call: spsa2turbo_powersave_bias_init_cpu, cpu: %d \n", cpu);
...
#define FREQ_MAX_AMOUNT_1 12
#define FREQ_MAX_AMOUNT_2 19
...
static int determine_new_freq(struct cpufreq_policy* policy, struct spsa2_policy_dbs_info* dbs_info, struct od_dbs_tuners *od_tuners, unsigned int cluster, unsigned int load)
{
    ...
    model = calculate_functional(load, index, freq, en, freq_amount, target_load);
    model = load < target_load ? (-1 * model) : model;
    gradient = 0;
    old_dir = dbs_info->direction;
    
    if ((dbs_info->direction == 3) && (model < 0))
    {
        dbs_info->direction = -1;
    }    
    

    if ((dbs_info->direction == 0) || (model == 0) || (dbs_info->direction * model < 0))
    {
        if (model == 0)
        {
            gradient = 0;
            dbs_info->direction = 0;
        }

        else
        {
            dbs_info->direction = model > 0 ? 1 : -1;
            gradient = model / 2;
            
            if (gradient == 0)
                gradient = 1 * dbs_info->direction;
        }
    }
    
    else
        if (dbs_info->direction < 0)
        {            
            int addition;
            addition = 0;

            switch(dbs_info->direction) 
            {
                ...
                case -3:
                    ...
                    gradient = model - alpha;
                    dbs_info->direction = -3;
                    break;
    ...
    new_index = index + gradient;
    
     pr_warn("gov_spsa2, result cpu %u -- load: %u, t_load: %u, old_d: %d, n_d: %d, al: %d, grad: %d, old_i: %d, new_i: %d\n", policy->cpu, load, target_load, old_dir, dbs_info->direction, alpha, gradient, index, new_index);
...
static void od_check_cpu(int cpu, unsigned int load)
{
    ...
    //pr_warn("gov spsa2 call: od_check_cpu() cpu: %d \n", cpu);
	
	freq_next = determine_new_freq(policy, dbs_info_spsa, dbs_data->tuners, cluster, load);
...
static void od_dbs_timer(struct work_struct *work)
{
	...
	if (sample_type == OD_SUB_SAMPLE) {
        ...
        pr_warn("gov spsa2 BAD <od_dbs_timer> USED, freq changed to: %u \n", core_dbs_info->freq_lo);
...
static int od_init(struct dbs_data *dbs_data)
{
    ...
    {   
        pr_warn("gov spsa2 init spsa2turbo cluster: %d \n", cluster);
        freq = get_freq_array(cluster);    
        freq_amount = get_freq_amount(cluster);
...
static struct od_ops od_ops = {
	.powersave_bias_init_cpu = spsa2turbo_powersave_bias_init_cpu,
...
static void od_set_powersave_bias(unsigned int powersave_bias)
{
	...
	for_each_online_cpu(cpu) {
		...
		if (policy->governor != &cpufreq_gov_spsa2turbo)
			continue;
...
void od_register_powersave_bias_handler_spsa2turbo_copy(unsigned int (*f)
...
EXPORT_SYMBOL_GPL(od_register_powersave_bias_handler_spsa2turbo_copy);

void od_unregister_powersave_bias_handler_spsa2turbo_copy(void)
{
	od_ops.powersave_bias_target = generic_powersave_bias_target;
	od_set_powersave_bias(0);
}
EXPORT_SYMBOL_GPL(od_unregister_powersave_bias_handler_spsa2turbo_copy);
...
static
#endif
struct cpufreq_governor cpufreq_gov_spsa2turbo = {
	.name			= "spsa2turbo",
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	return cpufreq_register_governor(&cpufreq_gov_spsa2turbo);
}

static void __exit cpufreq_gov_dbs_exit(void)
{
    pr_warn("spsa2turbo exit cpufreq_gov_dbs_exit");
	cpufreq_unregister_governor(&cpufreq_gov_spsa2turbo);
}

MODULE_AUTHOR("Venkatesh Pallipadi <venkatesh.pallipadi@intel.com>");
MODULE_AUTHOR("Alexey Starikovskiy <alexey.y.starikovskiy@intel.com>");
MODULE_DESCRIPTION("'cpufreq_spsa2turbo' - A dynamic cpufreq governor for "
	"Low Latency Frequency Transition capable processors");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_SPSA2TURBO
