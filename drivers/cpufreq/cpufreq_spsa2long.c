/*
 *  drivers/cpufreq/cpufreq_spsa2long.c
 *
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_SPSA2LONG
static struct cpufreq_governor cpufreq_gov_spsa2long;
#endif

static unsigned int default_powersave_bias;

static void spsa2long_powersave_bias_init_cpu(int cpu)
{
	struct od_cpu_dbs_info_s *dbs_info = &per_cpu(od_cpu_dbs_info, cpu);

    pr_warn("gov spsa2 call: spsa2long_powersave_bias_init_cpu, cpu: %d \n", cpu);
...
//	}
    ...
    switch(dbs_info->phase) 
    {
        ...
        case 0:
            ...
            dbs_info->old_index = index + dbs_info->delta * betta;

            dbs_info->minus_model = calculate_functional(load, index, freq, en, freq_amount, target_load);               
            
            // plus measurement
            new_index = dbs_info->old_index + dbs_info->delta * betta; // == index
        ...
        case 1:
            ...
            gradient = (alpha * (difference)) / (2 * dbs_info->delta * betta);
            ...
            pr_warn("gov_spsa2, result cpu %u p:%u -- load: %u, t_load: %u,  plus: %d, minus: %d, al: %d, bt: %d, del: %d, grad: %d, old_i: %d, new_i: %d\n", policy->cpu, dbs_info->phase, load, target_load, dbs_info->plus_model, dbs_info->minus_model, alpha, betta, dbs_info->delta, gradient, dbs_info->old_index, new_index);
...
static int od_init(struct dbs_data *dbs_data)
{
    ...
    {   
        pr_warn("gov spsa2 init spsa2long cluster: %d \n", cluster);
        freq = get_freq_array(cluster);    
        freq_amount = get_freq_amount(cluster);
...
static struct od_ops od_ops = {
	.powersave_bias_init_cpu = spsa2long_powersave_bias_init_cpu,
...
static void od_set_powersave_bias(unsigned int powersave_bias)
{
	...
	for_each_online_cpu(cpu) {
		...
		if (policy->governor != &cpufreq_gov_spsa2long)
			continue;
...
void od_register_powersave_bias_handler_spsa2long_copy(unsigned int (*f)
...
EXPORT_SYMBOL_GPL(od_register_powersave_bias_handler_spsa2long_copy);

void od_unregister_powersave_bias_handler_spsa2long_copy(void)
{
	od_ops.powersave_bias_target = generic_powersave_bias_target;
	od_set_powersave_bias(0);
}
EXPORT_SYMBOL_GPL(od_unregister_powersave_bias_handler_spsa2long_copy);
...
static
#endif
struct cpufreq_governor cpufreq_gov_spsa2long = {
	.name			= "spsa2long",
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	return cpufreq_register_governor(&cpufreq_gov_spsa2long);
}

static void __exit cpufreq_gov_dbs_exit(void)
{
    pr_warn("spsa2long exit cpufreq_gov_dbs_exit");
	cpufreq_unregister_governor(&cpufreq_gov_spsa2long);
}

MODULE_AUTHOR("Venkatesh Pallipadi <venkatesh.pallipadi@intel.com>");
MODULE_AUTHOR("Alexey Starikovskiy <alexey.y.starikovskiy@intel.com>");
MODULE_DESCRIPTION("'cpufreq_spsa2long' - A dynamic cpufreq governor for "
	"Low Latency Frequency Transition capable processors");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_SPSA2LONG
